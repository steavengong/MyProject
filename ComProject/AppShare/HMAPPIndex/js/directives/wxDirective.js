...
.directive("wxHead",function(){
    ...
    $wxHead.replace = true;
    $wxHead.controller = ['$scope','$window','$element',function($scope, $window, $element){
        ...
        $scope.onApp = function(){
            var appDownload="http://a.app.qq.com/o/simple.jsp?pkgname=com.cmbb.smartmarket";
...
.directive("wxFoot",function(){
    ...
    $wxFoot.replace = true;
    $wxFoot.controller = ['$scope','$window','$element',function($scope, $window, $element){
        $scope.checkWx = function(){
            var ua = window.navigator.userAgent.toLowerCase();
            if(ua.match(/MicroMessenger/i) == 'micromessenger'){
                return true;
