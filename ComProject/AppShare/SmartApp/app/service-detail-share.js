...
var cgiPath = config.requestWXUrl;
var servicePagePath = window.location.href.split('#')[0];
...
$(document).on("pageInit","#page-service-detail",function(e,id,page){
    ...
    $("#page-service-detail").on("infinite", function() {
        if (loading) return;
        loading = true;
        ...
        uAjax.ajax(action,data,function(result){
            ...
            if(result.response.data.rows.length == 0){
                ...
                for(var i = 0 ;i < rows.length ; i++){
                    ...
                    var evaluteDescirption = $("<div class='item-descirption'><p>" + row.evaluateContent + "</p></div>");
    ...
    function searchService(action,data){
        uAjax.ajax(action,data,function(result){
            setTimeout(function(){
                ...
                $(".service-own-phone-group").attr({"data-phone":response.servicePhone,"href":"tel://" + response.servicePhone});
                ...
                imgUrl = response.servicesImg;
                link = servicePagePath;
    ...
    /*var iswx = '<%=request.getParameter("iswx")%>';
     var id = '<%=request.getParameter("id")%>';*/
    function browserRedirect() {
        if(!navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)){
            window.location.href = "http://www.mengbp.com/web/front/servicesDetail/"+serviceID+"/page.html";
        }
    }
    browserRedirect();
    if(iswx()){
        $('#attentions').css('display','block');
    }else{
        $('#attentions').css('display','none');
    }
